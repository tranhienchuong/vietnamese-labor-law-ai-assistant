...
class LegalGraphExpanderTests(unittest.TestCase):
    ...
    def test_minor_worker_policy_uses_multidocument_depth_and_references(self) -> None:
        ...
        self.assertEqual(expander.expansion_depth_for_intent(intent), 2)
    ...
    def test_retirement_contract_and_dispute_policy_classification(self) -> None:
        ...
        self.assertIn("retirement_age_table_lookup", classify_graph_query_intent(retirement))
        self.assertTrue(
    ...
    def test_profile_classification_prefers_graph_for_dispute_but_not_user_style_fact_pattern(self) -> None:
        dispute = route_query("Toi muon kien vi bi sa thai thi co can hoa giai truoc khi ra Toa khong?")
        user_style = route_query("Cong ty no luong 2 thang, toi nghi ngay duoc khong?")

        self.assertEqual(classify_graph_expansion_profile(dispute), "graph_favored")
        self.assertEqual(classify_graph_expansion_profile(user_style), "paraphrased_real_user")

    def test_paraphrased_real_user_profile_reduces_graph_breadth_when_seed_confidence_is_low(self) -> None:
        store = FakeGraphStore(GraphExpansionResult(seed_chunk_ids=("seed",), expanded_chunk_ids=()))
        expander = Neo4jLegalGraphExpander(
            store=store,
            config=LegalGraphConfig(enabled=True, complex_query_only=True, expansion_depth=4, max_expanded_chunks=16),
        )
        intent = route_query("Cong ty no luong 2 thang, toi nghi ngay duoc khong?")
        weak_record = RetrievedRecord(
            chunk_id="seed",
            parent_chunk_id=None,
            citation_text="Mau 02/PLI",
            text="bieu mau",
            dense_text="",
            sparse_text="",
            payload={"chunk_id": "seed", "document_id": "nghi-dinh-145-2020-nd-cp", "article_number": "48"},
        )

        expander.expand_from_hits(
            hits=(make_hit("seed", 0.3),),
            direct_records={"seed": weak_record},
            intent=intent,
        )

        self.assertEqual(store.calls[0][1]["depth"], 2)
        self.assertEqual(store.calls[0][1]["limit"], 4)
        self.assertGreaterEqual(float(store.calls[0][1]["min_confidence"]), 0.75)

    def test_hard_negative_profile_filters_graph_only_hits_without_issue_match(self) -> None:
        expander = Neo4jLegalGraphExpander(
            store=FakeGraphStore(GraphExpansionResult(seed_chunk_ids=(), expanded_chunk_ids=())),
            config=LegalGraphConfig(enabled=True, complex_query_only=True),
        )
        intent = route_query("Lam them vao ngay le thi it nhat duoc tra bao nhieu phan tram, dung nham voi gioi han lam them.")
        hits = (
            SearchHit(
                chunk_id="graph-bad",
                qdrant_point_id="graph-bad",
                score=0.4,
                citation_text="BLLD Dieu 107",
                payload={
                    "chunk_id": "graph-bad",
            ...
            SearchHit(
                chunk_id="graph-good",
                qdrant_point_id="graph-good",
                score=0.4,
                citation_text="BLLD Dieu 98",
                payload={
                    "chunk_id": "graph-good",
                    "retrieval_source": "graph",
                    "retrieval_method": "neo4j_graph_expansion",
        ...
        records = {
            "graph-bad": RetrievedRecord(
                "graph-bad",
                None,
                "BLLD Dieu 107",
                "gioi han lam them gio",
                "",
                "",
                {"chunk_id": "graph-bad", "document_id": "45-2019-qh14", "article_number": "107", "issue_type": ["lam_them_gio"], "topic": ["thoi_gio_lam_viec"]},
            ),
            "graph-good": RetrievedRecord(
                "graph-good",
                None,
                "BLLD Dieu 98",
                "tien luong lam them gio vao ngay le",
                "",
                "",
                {"chunk_id": "graph-good", "document_id": "45-2019-qh14", "article_number": "98", "issue_type": ["tien_luong"], "topic": ["thoi_gio_lam_viec"]},
            ),
        }

        filtered = expander.filter_expanded_hits_for_intent(hits, records, intent)

        self.assertIn("graph-good", [hit.chunk_id for hit in filtered])
