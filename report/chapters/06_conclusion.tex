...
This thesis presented an integrated, citation-grounded legal QA system bounded to a six-document Vietnamese labor-law corpus. The system combines legal corpus processing, Qdrant hybrid retrieval, Neo4j graph expansion, GPT-OSS 120B grounded answer generation in the deployed runtime, deterministic citation validation, an API backend, a frontend, and reproducible evaluation. The core result is not a new language model, but an engineering workflow for preserving legal evidence from corpus preparation to final answer validation.
