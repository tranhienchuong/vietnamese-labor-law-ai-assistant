...
function outcomeBadge(value: EvaluationRecord["answerCorrect"]) {
  if (value === "yes") return <Badge variant="success">Đúng</Badge>
  if (value === "partial") return <Badge variant="warning">Một phần</Badge>
  return <Badge variant="destructive">Sai</Badge>
}
...
export default function AdminEvaluationPage() {
  return (
    <AppPageShell
      ...
      description="Theo dõi benchmark, ground truth, hallucination và mức độ đúng của citation."
      title="Đánh giá"
    >
          ...
          <table className="w-full min-w-[1100px] text-left text-sm">
            <thead className="bg-muted text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold">Câu hỏi benchmark</th>
                <th className="px-4 py-3 font-semibold">Ground truth / citation</th>
                <th className="px-4 py-3 font-semibold">Câu trả lời hiện tại</th>
            ...
            <tbody className="divide-y divide-border">
              {EVALUATION_ROWS.map((row) => (
                <tr className="align-top hover:bg-muted/50" key={row.id}>
                  ...
                  <td className="max-w-sm px-4 py-4 leading-6">{row.aiAnswer}</td>
