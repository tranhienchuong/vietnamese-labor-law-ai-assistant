  ...
  if (citations.length === 0) {
    return (
      <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-sm text-warning-foreground">
        Chưa tìm thấy nguồn pháp lý đủ chắc chắn cho câu hỏi này.
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">Nguồn pháp lý</h3>
        <span className="text-xs text-muted-foreground">{citations.length} nguồn</span>
      </div>
      <div className="space-y-2">
            ...
            <div className="border-t border-border px-3 py-3 text-sm leading-6 text-muted-foreground">
              ...
              {citation.url ? (
                <a
                  ...
                  target="_blank"
                >
                  Xem đầy đủ <ExternalLink className="h-3.5 w-3.5" />
                </a>
              ) : null}
            </div>
          </details>
